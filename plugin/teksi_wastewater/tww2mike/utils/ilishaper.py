...
def create_ili(schema, model, log_path, recreate_schema=False):
    ...
    logger.info(f"ILISHAPER SCHEMAIMPORT INTO {schema}...")
    pgconf = get_pgconf()
    exec_(
        f'"{config.JAVA}" -jar {config.ILISHAPER} --config {config.ILISHAPER_CONFFILE}--out {config.ILISHAPER_MODEL_NAME}.ili --log {log_path} --trace {config.DSS_MODEL_NAME}'
    )
