CREATE OR REPLACE VIEW tww_hydr.vw_err_rp_level
 AS
	SELECT rp.obj_id,
		   rp.level,
		   wn.bottom_level,
...
-- View: tww_hydr.vw_err_primary2secondary
...
-- DROP VIEW tww_hydr.vw_err_primary2secondary;

-- View: tww_hydr.vw_err_primary2secondary

-- DROP VIEW tww_hydr.vw_err_primary2secondary;

CREATE OR REPLACE VIEW tww_hydr.vw_err_primary2secondary
 AS
 SELECT rp.obj_id,
   ...
   FROM tww_od.reach_point rp
     LEFT JOIN tww_od.reach re ON re.fk_reach_point_from::text = rp.obj_id::text OR re.fk_reach_point_to::text = rp.obj_id::text
	 LEFT JOIN tww_od.wastewater_networkelement ne_re ON ne_re.obj_id::text = re.obj_id::text
	 LEFT JOIN tww_od.channel ch ON ch.obj_id=ne_re.obj_id
     LEFT JOIN tww_od.wastewater_node wn ON wn.obj_id::text = rp.fk_wastewater_networkelement::text
...
-- DROP VIEW tww_hydr.vw_err_reach;

CREATE OR REPLACE VIEW tww_hydr.vw_err_reach
 AS
...
from tww_od.reach re
	...
	LEFT JOIN tww_vl.channel_function_hierarchic wn_to_fhi ON wn_to._function_hierarchic=wn_to_fhi.code
	LEFT JOIN tww_vl.channel_function_hierarchic wn_from_fhi ON wn_from._function_hierarchic=wn_from_fhi.code
...
-- View: tww_hydr.vw_err_catchment
--  Visualisiert Einzugsgebiete, der Anschlussknoten nicht PAA sind

-- DROP VIEW tww_hydr.vw_err_catchment;

CREATE OR REPLACE VIEW tww_hydr.vw_err_catchment
 AS
 ...
 SELECT ca.obj_id,
   ...
   FROM tww_od.catchment_area ca
     LEFT JOIN tww_od.vw_wastewater_node wn_swc ON wn_swc.obj_id::text = ca.fk_wastewater_networkelement_ww_current::text
     LEFT JOIN tww_od.vw_wastewater_node wn_rwc ON wn_rwc.obj_id::text = ca.fk_wastewater_networkelement_rw_current::text
     LEFT JOIN tww_od.vw_wastewater_node wn_swp ON wn_swp.obj_id::text = ca.fk_wastewater_networkelement_ww_planned::text
     LEFT JOIN tww_od.vw_wastewater_node wn_rwp ON wn_rwp.obj_id::text = ca.fk_wastewater_networkelement_rw_planned::text
